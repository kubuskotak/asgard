...
func TestS256Challenge(t *testing.T) {
	...
	for i, scenario := range scenarios {
		result := S256Challenge(scenario.code)
...
func TestEncrypt(t *testing.T) {
	...
	for i, scenario := range scenarios {
		result, err := Encrypt([]byte(scenario.data), scenario.key)
		...
		// try to decrypt
		if result != "" {
			decrypted, _ := Decrypt(result, scenario.key)
...
func TestDecrypt(t *testing.T) {
	...
	for i, scenario := range scenarios {
		result, err := Decrypt(scenario.cipher, scenario.key)
